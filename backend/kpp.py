...
def generate_climate_map():
    try:
        ...
        if interpolation_type == 'spatial':
            image, max_time_step, choropleth_data = generate_spatial_map(df, technique)
        else:  # temporal
            image, max_time_step, choropleth_data = generate_temporal_map(df, technique, time_step)
        ...
        return jsonify({
            ...
            'choroplethData': choropleth_data
        })
    except Exception as e:
...
def generate_spatial_map(df, technique):
    ...
    gdf = gpd.GeoDataFrame(
        df_time, geometry=gpd.points_from_xy(df_time.longitude, df_time.latitude)
    )
    choropleth_data = gdf.to_json()

    return Image.open(buf), max_time_step, choropleth_data
