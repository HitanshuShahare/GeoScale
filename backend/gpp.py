...
def generate_climate_map():
    try:
        ...
        if interpolation_type == 'spatial':
            image, max_time_step, grid_x, grid_y, z_interpolated = generate_spatial_map(df, technique)
        else:  # temporal
            image, max_time_step, grid_x, grid_y, z_interpolated = generate_temporal_map(df, technique, time_step)
...
def generate_spatial_map(df, technique):
    ...
    plt.contourf(grid_x, grid_y, z_interpolated, cmap='viridis')
    ...
    return Image.open(buf), max_time_step, grid_x, grid_y, z_interpolated
...
def generate_temporal_map(df, technique, time_step):
    ...
    for i in range(grid_x.shape[0]):
        for j in range(grid_x.shape[1]):
            # Find nearest point instead of exact match
...
def downscale():
    try:
        ...
        z_interpolated = np.array(data['z_interpolated'])
        
        fine_grid_x, fine_grid_y = np.meshgrid(np.linspace(grid_x.min(), grid_x.max(), 200), np.linspace(grid_y.min(), grid_y.max(), 200))
        
        # Use Radial Basis Function for downscaling
        rbf = interpolate.Rbf(grid_x.flatten(), grid_y.flatten(), z_interpolated.flatten(), function='multiquadric')
        z_downscaled = rbf(fine_grid_x, fine_grid_y)

        plt.figure(figsize=(10, 6))
        plt.contourf(fine_grid_x, fine_grid_y, z_downscaled, cmap='viridis')
        plt.colorbar(label='Temperature (°C)')
        plt.title('Downscaled Climate Map')
        plt.xlabel('Longitude')
        plt.ylabel('Latitude')
        ...
        logger.error(f"Error in downscale: {str(e)}")
        return jsonify({'error': str(e)}), 400
